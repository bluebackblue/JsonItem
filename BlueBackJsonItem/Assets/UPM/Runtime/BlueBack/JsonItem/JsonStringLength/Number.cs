		...
		*/
		public static int GetLength(string a_string,int a_index,int a_length)
		{
			if(a_index < a_length){
					...
					default:
						{
							...
							DebugTool.Assert(false,string.Format("index = {0} : char = {1} : {2}",t_index,a_string[t_index],a_string));
							#endif
				...
				#if(DEF_BLUEBACK_DEBUG_ASSERT)
				DebugTool.Assert(false,string.Format("index = {0} : char = {1} : {2}",a_index,a_string[a_index],a_string));
				#endif
